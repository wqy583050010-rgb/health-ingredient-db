...
const statusConfig: Record<ComplianceStatus, { icon: React.ReactNode; label: string; className: string }> = {
  approved: { icon: <CheckCircle className="w-3.5 h-3.5" />, label: '已批准', className: 'bg-teal-100 text-teal-700' },
  restricted: { icon: <AlertTriangle className="w-3.5 h-3.5" />, label: '受限', className: 'bg-amber-100 text-amber-700' },
  prohibited: { icon: <XCircle className="w-3.5 h-3.5" />, label: '未批准', className: 'bg-red-100 text-red-700' },
  pending: { icon: <Clock className="w-3.5 h-3.5" />, label: '审批中', className: 'bg-gray-100 text-gray-600' },
};
